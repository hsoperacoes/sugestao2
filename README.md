...
<head>
  ...
  <style>
    :root{
      ...
      --radius:18px; --shadow:0 12px 34px rgba(17,25,40,.28);
    }
    *{box-sizing:border-box}
    ...
    header .logo{width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#58c1ff,transparent),#0f172a;display:grid;place-items:center;box-shadow:var(--shadow)}
    ...
    .card{background:var(--card);border:1px solid #1d2941;border-radius:var(--radius);padding:18px;box-shadow:var(--shadow);position:relative}
    .card h3{margin:0 0 10px;font-size:18px}
    .card .muted{color:var(--muted);font-size:14px}
    .scroll{max-height:360px;overflow:auto;border:1px solid #22314d;border-radius:12px}

    @media(min-width:1000px){
      #home .grid{display:grid;grid-template-columns:3fr 2fr;gap:16px}
    }

    /* Choose branch card (rodapé) */
    .choose-bar{display:grid;place-items:center;margin-top:18px}
    .choose-card{display:inline-flex;align-items:center;gap:10px;background:#0f1626;border:1px solid #2a3a5b;border-radius:14px;padding:12px 16px;cursor:pointer;transition:.2s}
    .choose-card:hover{transform:translateY(-1px);box-shadow:0 10px 30px rgba(0,0,0,.35)}
    .choose-card i{color:var(--brand)}

    /* Modal */
    .overlay{position:fixed;inset:0;background:rgba(7,12,22,.55);backdrop-filter:blur(4px);display:none;z-index:40}
    .overlay.show{display:block}
    .modal{position:fixed;inset:0;display:none;place-items:center;z-index:50}
    .modal.show{display:grid}
    .modal-card{width:min(560px,92vw);background:#0f1626;border:1px solid #2a3a5b;border-radius:18px;padding:22px;box-shadow:var(--shadow);animation:pop .18s ease-out}
...
<body>
  <!-- HOME (mostra combos/mural e o card de escolher filial). Some após a escolha. -->
  ...
  <!-- PORTAL (aparece só depois de escolher a filial) -->
  ...
  <!-- BACKDROP + MODAL (seleção de filial) -->
